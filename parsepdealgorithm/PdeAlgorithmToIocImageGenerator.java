 ...
 */
public class PdeAlgorithmToIocImageGenerator {
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            proc.write(out, prg);
            String classContent = out.toString().replaceFirst("extends PApplet \\{"
            ...
            fout = new FileWriter(hashArgs.get(CLASS_NAME_ARG)+".java");            
            fout.write(classContent);            
            out.flush();
            
        } catch (SketchException ex) {
            ret=-1;
            Logger.getLogger(PdeAlgorithmToIocImageGenerator.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private static void setArgs(String[] args){
        ...
        if(!hashArgs.containsKey(CLASS_NAME_ARG)){
            if(hashArgs.containsKey(PDE_FILE_ARG)){
                String cn = hashArgs.get(PDE_FILE_ARG);
